...
void SimpleShapeApplication::init()
{
    ...
    glBufferData(GL_UNIFORM_BUFFER, 12 * sizeof(float), nullptr, GL_STATIC_DRAW);        // base alignment  // aligned offset
    glBufferSubData(GL_UNIFORM_BUFFER, 0, 2 * sizeof(float), &scale);                    // 8               // 0
    glBufferSubData(GL_UNIFORM_BUFFER, 2 * sizeof(float), 2 * sizeof(float), &trans);    // 8               // 8
    glBufferSubData(GL_UNIFORM_BUFFER, 4 * sizeof(float), 2 * sizeof(float), &rot[0]);   // 16              // 16
    glBufferSubData(GL_UNIFORM_BUFFER, 8 * sizeof(float), 2 * sizeof(float), &rot[1]);   // 16              // 32
    glBindBufferBase(GL_UNIFORM_BUFFER, 1, transformation_buffer_handle);                // needed mem 48
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
